...
- **Publication History & CSV Export (v1.6.5)**: A dedicated "History" page lists all processed and ignored posts with filtering by status, source page, and date range. Displays specific publication statuses (Published, Draft, Scheduled, Ignored, or Processed) by querying the linked WordPress post status dynamically. Provides direct WordPress view/edit links inside the original URL table cell and as a dedicated **URL WordPress** column in the CSV export.
